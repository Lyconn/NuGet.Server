...
namespace NuGet.Server.DataServices {
    public class PackagesODataController : NuGetODataController {
        ...
        public async Task<HttpResponseMessage> ClearCache(CancellationToken token) {
            if (this.RequestContext.IsLocal || ServiceResolver.Current.Resolve<ISettingsProvider>().GetBoolSetting("allowRemoteCacheManagement", false)) {
                await this._serverRepository.ClearCacheAsync(token);
                return this.CreateStringResponse(HttpStatusCode.OK, "Server cache has been cleared.");
            } else {
                return this.CreateStringResponse(HttpStatusCode.Forbidden, "Clear cache is only supported for local requests.");
            }
        }
    }
}